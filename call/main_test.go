...
func des(curTransIndex int, UserLast100Trans []int) {

	if curTransIndex >= 9 { //last 10
		fmt.Println("last 9")
		for i := curTransIndex - 1; i >= curTransIndex-9; i-- { //last 9
			fmt.Println(UserLast100Trans[i])
		}
		//cur trans index is last one
		fmt.Println("last 1")
		fmt.Println(UserLast100Trans[curTransIndex])
		fmt.Println("last 90")
		for i := curTransIndex + 1; i < len(UserLast100Trans); i++ {
			fmt.Println(UserLast100Trans[i])
		}
		for i := 0; i < curTransIndex-9; i++ {
			fmt.Println(UserLast100Trans[i])
		}

	} else {
		fmt.Println("last 9")
		for i := 0; i < curTransIndex; i++ { //last 9
			fmt.Println(UserLast100Trans[i])
		}
		//cur trans index is last one
		fmt.Println("last 1")
		fmt.Println(UserLast100Trans[curTransIndex])
		fmt.Println("last 90")
		for i := curTransIndex + 1; i <= curTransIndex+len(UserLast100Trans)-10; i++ {
			fmt.Println(UserLast100Trans[i])
		}
		fmt.Println("last 9")
		if len(UserLast100Trans)-9 > 0 {
			for i := curTransIndex + len(UserLast100Trans) - 9; i < len(UserLast100Trans); i++ {
				fmt.Println(UserLast100Trans[i])
			}
		}
